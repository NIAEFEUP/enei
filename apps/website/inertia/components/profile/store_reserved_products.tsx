...
export default function StoreReservedProducts({ products }: StoreReservedProductsProps) {
  return (
    <>
      {products?.map(({ product, order }) => (
        <PointsStoreProductCollectCard key={product.id} product={product} status={order.status} />
      ))}
    </>
  );
}
