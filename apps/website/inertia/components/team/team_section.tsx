...
// 1272 1240
export default function TeamSection({ department, people }: TeamSectionProps) {
  const sortedPeople = useMemo(() => {
    const copy = [...people];
    return copy.toSorted(
      (a, b) => (b.staffProfile.isCoordinator ? 1 : 0) - (a.staffProfile.isCoordinator ? 1 : 0),
    );
  }, [people]);
  return (
    <section className="flex flex-col items-center justify-center gap-y-4">
      ...
      <div className="max-w-6xl">
        <div className="max-w-2/6 mt-4 flex w-full flex-row flex-wrap justify-center gap-x-6 gap-y-8">
