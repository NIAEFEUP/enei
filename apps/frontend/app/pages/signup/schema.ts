...
const university = z
  .string()
  .nonempty({ message: "A universidade é obrigatória." })
...
const termsAndConditions = z
  .boolean()
  .refine((val) => val, { message: "Tens de concordar com os termos e condições" });
...
export const personalInfoSchema = z.object({
  ...
  lastName: z.string().nonempty({ message: "O último nome é obrigatório." }),

  dateOfBirth: z.date({ message: "A data de nascimento é obrigatória." }).max(new Date()),

  phone: z.string().nonempty({ message: "O número de telefone é obrigatório." }),

  municipality: z
    .string({ message: "Campo obrigatório" })
    .nonempty({ message: "O município deve ter pelo menos 1 letra" }),
});
...
export const communicationsInfoSchema = z.object({
  ...
  attendedBeforeEditions: z.array(z.object({ label: z.string(), value: z.string() })).optional(),
  termsAndConditions,
});
