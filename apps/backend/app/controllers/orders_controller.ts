  ...
  public async createMBWay({ request, auth, response }: HttpContext) {
    ...
    try {
      ...
      for (const { product, quantity } of productDetails) {
        await OrderProduct.create({ orderId: order.id, productId: product.id, quantity });
      }
      ...
      if (apiResponse.status === 200) {
        ...
        return response.status(200).json({ order, message: "Payment initiated successfully" });
      } else {
      ...
      return response
        .status(500)
        .json({ message: "An error occurred while initiating the payment" });
    }
  }
