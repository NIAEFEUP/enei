...
import { X as RemoveIcon } from "@enei/shadcn/icons";
...
const CarouselUploadContext = createContext<CarouselWithUploadContext | null>(null);
...
export function FileUploadCarouselProvider<T>({
  ...
  const addImageToTheSet = useCallback(
    (file: File) => {
      ...
      const fileWithPreview = { file, preview: URL.createObjectURL(file) } as T;
      onValueChange((prev) => {
  ...
  return (
    <CarouselUploadContext.Provider
      value={{ addImageToTheSet, removeImageFromPreview, isFileTooBig, dropzoneState, isLOF }}
    >
      <div
        tabIndex={0}
...
export const SliderThumbItemWithRemove = forwardRef<
  HTMLButtonElement,
  { index: number } & React.HTMLAttributes<HTMLButtonElement>
  ...
  return (
    <SliderThumbItem index={index} className={className}>
      <button
        ...
        {...props}
        type="button"
        className={cn("absolute -right-2 -top-1 z-10 h-6 w-6 opacity-70 focus:outline-none")}
...
export const CustomUploadInput = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, children, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        {...props}
        className={`w-full ${isLOF ? "cursor-not-allowed opacity-50" : "cursor-pointer"}`}
      >
        <div
          className={cn(
            `border-muted-foreground w-full rounded-lg border border-dashed duration-300 ease-in-out ${
